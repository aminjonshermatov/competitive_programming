...
inline auto mod(auto a, auto mod) noexcept {
...
void solve() {
  ...
  for (int i = 1; i <= q; ++i) {
    i64 a, b, c;
    std::cin >> a >> b >> c;
    const auto A = 1 + mod(mod(a * (1 + X), 998244353), 2);
    const auto B = 1 + mod(mod(b * (1 + X), 998244353), n);
    const auto C = 1 + mod(mod(c * (1 + X), 998244353), n);

    if (A == 1) {
      lct.link(vs[B - 1], vs[C - 1]);
    } else {
      if (lct.get_root(vs[B - 1]) != lct.get_root(vs[C - 1])) {
        X = 0;
      } else {
        lct.expose(vs[B - 1]);
        const auto x = lct.get_root(vs[C - 1]);
        if (x == nullptr) {
          X = 0;
        } else {
          const auto y = lct.get_parent(x);
          X = y == nullptr || y != vs[B - 1] ? 0 : lct.get_key(x);
        }
      }
      std::cout << X << '\n';
    }
