...
void solve() {
  ...
  vector<ll> ans(n);
  rep(i, n) {
    sort(all(C), [&](point2d &lhs, point2d &rhs) {
      auto l_d = abs(S[i].x - lhs.x) + abs(S[i].y - lhs.y);
      auto r_d = abs(S[i].x - rhs.x) + abs(S[i].y - rhs.y);
      if (l_d == r_d) return lhs.idx < rhs.idx;
      return l_d < r_d;
    });
    ans[i] = C[0].idx;
  }
