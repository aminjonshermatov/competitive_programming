...
void solve() {
    ...
    n = sz(A);
    vector<vector<int>> seen(x, vector<int>(n, -1));

    auto dp = [&](auto f, ll s, int idx) -> bool {
        if (s >= x) return s == x;
        if (idx >= n) return false;
        if (seen[s][idx] != -1) return seen[s][idx];

        return seen[s][idx] = f(f, s + A[idx], idx + 1) || f(f, s, idx + 1);
    };
    cout << (dp(dp, 0, 0) ? "Yes" : "No") << '\n';
}
