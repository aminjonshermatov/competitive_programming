...
void solve() {
    string s; cin >> s;
    auto is_up = [](auto ch) { return clamp(ch, 'A', 'Z') == ch; };
    auto is_digit = [](auto ch) { return clamp(ch, '0', '9') == ch; };
    if (sz(s) != 8 || !is_up(s.front()) || !is_up(s.back()) || s[1] == '0') {
        cout << "No";
        return;
    }
    for (int i = 1; i < 7; ++i) if (!is_digit(s[i])) {
        cout << "No";
        return;
    }
    cout << "Yes";
