...
class Info {
 public:
  void Apply(const Tag& tag) {
    SumSq += tag.Val * (Mint(2) * Sum + tag.Val * Len);
    Sum += tag.Val * Len;
  }

 public:
  Mint Sum = 0, SumSq = 0, Len = 1;
};
Info operator+(const Info& lhs, const Info& rhs) {
  return {lhs.Sum + rhs.Sum, lhs.SumSq + rhs.SumSq, lhs.Len + rhs.Len};
}
...
void Solve() {
  ...
  // (sum(b1 + b2 + ...))^2 - sum(b1^2 + b2^2 + ...)
  LazySegmentTree<Info, Tag> st(n);

  const auto inv2 = Mint(2).Inverse();
  while (q-- > 0) {
    int l, r, a;
    Scan(l, r, a);

    st.Modify(l - 1, r, {a});
    const auto [sum, sumSq, len] = st.Query(l - 1, r);
    Println((sum * sum - sumSq) * inv2);
  }
}
