...
void solve() {
    int n; cin >> n;
    map<string_view, vector<string>> g;
    vector<string> S(n);
    rep(i, n) {
        string t; cin >> S[i] >> t;
        g[S[i]].eb(t);
    ...
    map<string_view, int> color;
    auto dfs = [&](auto f, string_view v) -> bool {
        color[v] = 1;
        for (auto &u : g[v]) {
            auto &c = color[u];
            if (c == 1 || c == 0 && f(f, u)) return true;
        }
    ...
    rep(i, n) if (color[S[i]] == 0 && dfs(dfs, S[i])) { cout << "No"; return; }
    cout << "Yes";
}
