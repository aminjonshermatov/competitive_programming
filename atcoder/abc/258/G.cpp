...
inline constexpr int MAXN = 3'030;

array<bitset<MAXN>, MAXN> A;

void solve() {
    int n; cin >> n;

    vector<vector<bool>> adj(n, vector<bool>(n));
    rep(i, 0, n) rep(j, 0, n) { char x; cin >> x; if (i < j) { A[i][j] = adj[i][j] = x == '1'; } }

    ll ans = 0;
    rep(i, 0, n) rep(j, i + 1, n) if (adj[i][j]) ans += (A[i] & A[j]).count();
    cout << ans;
}
