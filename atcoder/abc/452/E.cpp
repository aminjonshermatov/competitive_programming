...
void Solve() {
  ...
  std::vector<Mint> pfX(n + 1), pfY(n + 1);
  for (int i = 1; i <= n; ++i) {
    pfX[i] = pfX[i - 1] + as[i];
    pfY[i] = pfY[i - 1] + as[i] * i;
  }

  Mint ret{};
  for (int j = 2; j <= m; ++j) {
    for (int i = 1; i <= n; i += j) {
      const auto r = std::min(n, i + j - 1);
      ret += (pfY[r - (r % j == 0)] - pfY[i - 1] - (pfX[r - (r % j == 0)] - pfX[i - 1]) * (i - 1)) * bs[j];
    }
  }
  Println(ret);
}
