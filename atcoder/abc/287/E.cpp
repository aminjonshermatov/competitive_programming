...
struct Trie {
    struct Node {
        array<Node*, 26> child{};
        int cnt;
        Node() : cnt(0) { fill(all(child), nullptr); }
    };
    ...
    void add(string_view s) {
        auto cur = root;
        for (auto ch : s) {
            if (cur->child[ch - 'a'] == nullptr) cur->child[ch - 'a'] = new Node();
            ++cur->child[ch - 'a']->cnt;
            cur = cur->child[ch - 'a'];
        }
    }

    int get_max(string_view s) {
        if (s.empty()) return 0;
        auto cur = root;
        int ans = 0;
        for (auto ch : s) {
            assert(cur->child[ch - 'a'] != nullptr);
            if (cur->child[ch - 'a']->cnt > 1) ++ans;
            else break;
            cur = cur->child[ch - 'a'];
        }
        return ans;
...
void solve() {
    ...
    Trie trie{};
    vector<string> A(n);
    forr(a, A) cin >> a, trie.add(a);
    forr(a, A) cout << trie.get_max(a) << '\n';
}
