...
void solve() {
    ...
    auto dfs = [&](auto f, int u, int p) -> bool {
        if (sz(g[u]) > 2 || g[u].empty()) return false;
        used[u] = true;
        if (p != -1 && sz(g[u]) == 1) return true;
        forr(v, g[u]) if (v != p && !used[v]) {
            return f(f, v, u);
        }
        return false;
    };
    ...
    vector<int> S;
    rep(u, n) if (sz(g[u]) == 1) S.eb(u);
    forr(u, S) {
        fill(all(used), false);
        if (dfs(dfs, u, -1) && all_of(all(used), [](auto x) { return x; })) {
            cout << "Yes";
            return;
        }
    }
