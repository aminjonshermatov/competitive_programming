...
void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    rows[r].emplace_back(c, x);
    col_sum[c] += x;
  }

  set<pair<ll, int>> best_col;
  for (auto [c, sm] : col_sum) {
    best_col.emplace(sm, c);
  }
  ...
  for (auto &[r, cs] : rows) {
    ...
    for (auto [c, x] : cs) {
      sm += x;
      best_col.erase(pair(col_sum[c], c));
      col_sum[c] -= x;
      best_col.emplace(col_sum[c], c);
    }
    ans = max(ans, sm + prev(best_col.end())->first);
    for (auto [c, x] : cs) {
      best_col.erase(pair(col_sum[c], c));
      col_sum[c] += x;
      best_col.emplace(col_sum[c], c);
