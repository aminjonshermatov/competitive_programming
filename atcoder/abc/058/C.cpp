...
inline constexpr int N = 26;

void solve() {
  int n; cin >> n;
  vector<array<int, N>> cnt(n);
  rep(i, n) fill(all(cnt[i]), 0);

  rep(i, n) {
    string s; cin >> s;
    forr(ch, s) ++cnt[i][ch - 'a'];
  }
  array<int, N> mn{};
  fill(all(mn), inf);
  rep(ch, N) {
    rep(i, n) mn[ch] = min(mn[ch], cnt[i][ch]);
  }
  string res;
  rep(ch, N) {
    if (mn[ch] > 0 && mn[ch] != inf) res.append(mn[ch], 'a' + ch);
  }
  cout << res << '\n';
}
