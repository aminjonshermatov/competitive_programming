...
void solve() {
    ...
    auto can = [&](ll lim) -> bool {
        ...
        rep(v, 0, n) forr(u, g[v]) cost[v] += A[u];
        rep(v, 0, n) if (cost[v] <= lim) q.push(v), used[v] = true;

        while (!q.empty()) {
            auto v = q.front(); q.pop();

            forr(u, g[v]) if (!used[u] && (cost[u] -= A[v]) <= lim) q.push(u), used[u] = true;
        }

        return all_of(all(used), [](auto b) { return b; });
    };
