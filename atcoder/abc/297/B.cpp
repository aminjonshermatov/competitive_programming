...
void solve() {
  string s; cin >> s;
  assert(count(s.begin(), s.end(), 'B') == 2);
  vector<int> B;
  for (int i = 0; i < s.size(); ++i) {
    if (s[i] == 'B') B.emplace_back(i);
  }

  if (B[0] % 2 == B[1] % 2 || B[0] > B[1]) {
    cout << "No\n";
    return ;
  }

  assert(count(s.begin(), s.end(), 'R') == 2);
  assert(count(s.begin(), s.end(), 'K') == 1);
  vector<int> RKR;
  for (int i = 0; i < s.size(); ++i) {
    if (s[i] == 'R' || s[i] == 'K') RKR.emplace_back(i);
  }

  if (!(s[RKR[0]] == 'R' && s[RKR[1]] == 'K' && s[RKR[2]] == 'R' && RKR[0] < RKR[1] && RKR[1] < RKR[2])) {
    cout << "No\n";
    return ;
  }
  cout << "Yes\n";
