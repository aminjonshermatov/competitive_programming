...
void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    std::cin >> T[i] >> X[i];
    add[T[i]].emplace_back(i);
    del[T[i] + d].emplace_back(i);
  }

  const int max_t = *std::max_element(T.begin(), T.end()) + d + 1;
  const int max_x = *std::max_element(X.begin(), X.end()) + w + 1;

  LazySegmentTree<Node, Tag> st(max_x);

  int ans = 0;
  for (int t = 0; t <= max_t; ++t) {
    for (auto i : del[t]) {
      st.modify(X[i], X[i] + w, {-1});
    }
    for (auto i : add[t]) {
      st.modify(X[i], X[i] + w, {+1});
    }
    ans = std::max(ans, st.query(0, max_x).val);
  }
  std::cout << ans << '\n';
}
