...
void solve() {
    int n; cin >> n;
    vector<pii> A(n);
    forr(a, A) cin >> a.fi >> a.se;
    array<vector<Mint>, 2> dp{};
    rep(i, 2) dp[i].resize(n);
    dp[0][0] = dp[1][0] = 1;

    for (int i = 1; i < n; ++i) {
        if (A[i].fi != A[i - 1].fi) dp[0][i] += dp[0][i - 1];
        if (A[i].fi != A[i - 1].se) dp[0][i] += dp[1][i - 1];
        if (A[i].se != A[i - 1].fi) dp[1][i] += dp[0][i - 1];
        if (A[i].se != A[i - 1].se) dp[1][i] += dp[1][i - 1];
    }
