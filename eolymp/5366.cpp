...
void solve() {
    ...
    vector<int> size(n, 0);
    auto dfs1 = [&](auto f, int v, int p) -> void {
        size[v] = 1;
    ...
    auto find_centroid = [&](auto f, int v, int p) -> int {
        for (auto u : g[v]) if (u != p && size[u] > n/2) return f(f, u, v);
        return v;
    };
    auto v = find_centroid(find_centroid, 0, -1);

    vector<int> res;
    res.eb(v);
    rep(i, n) if (i != v && size[i] * 2 == n) res.eb(i);
