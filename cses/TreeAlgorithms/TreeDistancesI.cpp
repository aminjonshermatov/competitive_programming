...
void solve() {
    ...
    auto dfs = [&](auto f, int v, int p, vector<int> &dist) -> void {
        pii mx(-1, v);
        forr(u, g[v]) if (u != p) {
            dist[u] = dist[v] + 1;
            f(f, u, v, dist);
            if (dist[u] > mx.fi) mx = pii(dist[u], u);
        }
    };

    dfs(dfs, 0, -1, dist1);
    auto x = max_element(all(dist1)) - dist1.begin();
    fill(all(dist1), 0);
    dfs(dfs, x, -1, dist1);
    auto y = max_element(all(dist1)) - dist1.begin();
    vector<int> dist2(n, 0);
    dfs(dfs, y, -1, dist2);
    vector<int> ans(n);
    rep(v, n) ans[v] = max(dist1[v], dist2[v]);
    rep(v, n) cout << ans[v] << " \n"[v + 1 == n];
}
