...
constexpr auto md = 1000001269ll;
...
inline constexpr array<string_view, 2> YN {"NO", "YES"};
...
void solve() {
  ...
  FenwickTreePointUpdateRangeQuery<Mint> f(n), b(n);
  for (int i = 0; i < n; ++i) {
    f.modify(i, pw[n - i - 1] * (s[i] - 'a' + 1));
  }
  for (int i = n - 1; i >= 0; --i) {
    b.modify(i, pw[i] * (s[i] - 'a' + 1));
  }
  ...
  while (q--) {
    ...
    if (cmd == '1') {
      ...
      cin >> k >> x;
      --k;
      f.modify(k, pw[n - k - 1] * (x - s[k]));
      b.modify(k, pw[k] * (x - s[k]));
      s[k] = x;
      dbg(s);
    } else if (cmd == '2') {
      int l, r; cin >> l >> r;
      --l;
      cout << YN[(f.query(l, r) / pw[n - r]) == (b.query(l, r) / pw[l])] << '\n';
    } else {
      assert(false);
    }
  }
