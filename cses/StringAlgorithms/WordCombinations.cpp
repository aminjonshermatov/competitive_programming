...
void solve() {
  ...
  i64 h = 0;
  for (int i = 0; i < n; ++i) {
    prf[i] = h = add(mul(h, P), t[i] - 'a' + 1);
  }

  auto get = [&](int l, int r) -> i64 {
    assert(l <= r && r < t.size());
    return sub(prf[r], l > 0 ? mul(prf[l - 1], pw[r - l + 1]) : 0);
  };

  unordered_map<int, unordered_map<i64, int>> hs;
  cin >> n;
  for (int i = 0; i < n; ++i) {
    string s; cin >> s;
    h = 0;
    for (auto ch : s) {
      h = add(mul(h, P), ch - 'a' + 1);
    }
    ++hs[int(s.size())][h];
  }
