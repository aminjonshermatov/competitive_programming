...
void solve() {
    ...
    rep(v, 0, n) if (sz(g[v]) & 1) { cout << "IMPOSSIBLE"; return; }
    ...
    while (!st.empty()) {
        ...
        while (!g[v].empty()) {
            auto [u, i] = g[v].back(); g[v].pop_back();
            if (!used[i]) { st.push(u); used[i] = true; ok = true; break; }
        }
        if (!ok) path.eb(v), st.pop();
    }

    if (sz(path) != m + 1) { cout << "IMPOSSIBLE"; return; }
