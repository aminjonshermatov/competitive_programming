...
void solve() {
  ...
  vector<vector<vector<BigInt>>> cache(n + 1, vector<vector<BigInt>>(n + 1, vector<BigInt>(n + 1, -1)));
  auto dp = [&](auto f, int open, int close, int mx) -> BigInt {
    if (open + close >= 2 * n) return open == n && close == n && mx == k;
    if (cache[open][close][mx] != -1) return cache[open][close][mx];
    BigInt ans = 0;
    if (open < n) ans += f(f, open + 1, close, max(mx, abs(open + 1 - close)));
    if (open > close) ans += f(f, open, close + 1, max(mx, abs(open - close - 1)));
    return cache[open][close][mx] = ans;
  };
