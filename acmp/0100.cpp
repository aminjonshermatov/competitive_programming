...
void solve() {
  int n; cin >> n;
  const int half = n / 2;
  const int lim = half * 9;
  vector<vector<BigInt>> dp(half, vector<BigInt>(lim + 1, 0)); // dp[first i indexes][sum] = number of ways to choice first {i} indexes with sum = {sum}
  for (int s = 0; s < 10; ++s) {
    dp[0][s] = 1;
  }
